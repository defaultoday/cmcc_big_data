        ...
        while True:
            try:
                ...
                if(check.text == text):
                    ...
                    #选择内容申诉
                    time.sleep(1)
                    check_list = self.browser.find_elements_by_xpath('/html/body/div[4]/div[1]/div[1]/ul/li[3]/span[text()="内容申诉"]')
                    check_list[0].click()
                    time.sleep(3)
                    #选择附件
                    ...
                    time.sleep(5)
                    #上传文件
                    path = os.getcwd() + '\\data\\申诉报告.docx'
                    self.upload(path)
                    time.sleep(3)
                    #内容申诉按钮
                    self.browser.find_element_by_xpath('/html/body/div[1]/div/div/div/div[2]/div/div/div[1]/div[2]/div[7]/button[3]/span').click()
                    time.sleep(3)
                    print("处理完成："+text)
                    self.out_file.write(text+'\n')
                    self.out_file.flush()
...
def appeal_stack(in_path="",out_path=""):
    start = Appeal(in_path=in_path,out_path=out_path)
    start.run()
...
if __name__ == "__main__":
    ...
    data.run()
    for i in range(0,4):
        send_thread = threading.Thread(target=appeal_stack,args=('./data/data'+str(i)+".txt","./data/Appeal_Thread"+str(i)+".txt"))

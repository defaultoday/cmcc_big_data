...
class Appealadopt(Base):
    """
    内容申诉类，选择内容申诉，选择上传附件，点击内容申诉
    """
        ...
        while True:
            try:
                ...
                if(check.text == text):
                    ...
                    #内容申诉按钮
                    self.browser.find_element_by_xpath('/html/body/div[1]/div/div/div/div[1]/div/div[4]/div/div[1]/div/div[2]/table/thead/tr/th[7]/div/div/span[2]/div/div/button').click()
                    time.sleep(3)
                    print("处理完成："+text)
                    self.out_file.write(text+'\n')
                    self.out_file.flush()
...
def appealadopt_stack(in_path="",out_path=""):
    start = Appealadopt(in_path=in_path,out_path=out_path)
    start.run()

if __name__ == "__main__":
    
    appealadopt_task = Appealadopt()
    appealadopt_task.run()
    """
    ...
    data.run()
    for i in range(0,4):
        send_thread = threading.Thread(target=appealadopt_stack,args=('./data/data'+str(i)+".txt","./data/Appealadopt_Thread"+str(i)+".txt"))

...
def submit_stack(in_path="",out_path=""):
    start = SubmitTask(in_path=in_path,out_path=out_path)
    start.run()
...
if __name__ == "__main__":
    ...
    data.run()
    for i in range(0,4):
        send_thread = threading.Thread(target=submit_stack,args=('./data/data'+str(i)+".txt","./data/Recv_Thread"+str(i)+".txt"))
